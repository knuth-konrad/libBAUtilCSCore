...
// using static libBAUtilCS.DateTimeUtil;
using static libBAUtilCoreCS.FilesystemUtil;
   ...
   class Program
   {
      static void Main(string[] args)
      {
         ...
         */
         ...
         Console.WriteLine(" * **DateTimeUtil");
         BlankLine();
      ...
      static void IOUtils()
      {
         ...
         sSource = "C:\\DATA\\CSharp\\zTEST\\libBAUtilCSTest\\DATA\\1.tmp";
         string sRet = sSource;

         for (Int32 i = 0; i <= 9; i++)
         {
            WriteIndent(System.String.Format("{0}) BackupFile {2}: {1}", System.String.Format("00", i),
      ...
      static void DateTimeUtils()
      {
         ...
         // * Constructors
         // Default (empty)
         DateTimeUtil dtm = new DateTimeUtil();


         WriteIndent("- New()", 1);
         WriteIndent(System.String.Format("- .ToDateIATA: {0}", dtm.ToDateIATA()), 2);
         WriteIndent(System.String.Format("- .ToDateIATA: {0}", dtm.ToDateIATA(DateTimeUtil.eIATADateType.DateLong)), 2);
         BlankLine();

         // DateTime
         dtm = new DateTimeUtil(new DateTime(2019, 4, 9));
         ...
         // Ticks
         dtm = new DateTimeUtil(9999999999999);
         ...
         // Year, Month, Day
         dtm = new DateTimeUtil(2019, 4, 10);
         ...
         {
            WriteIndent(System.String.Format("Month: {0}, last day in month: {1}", i, DateTimeUtil.GetLastDayInMonth(i, 2020).ToString()), 2);
         }
